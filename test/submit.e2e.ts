...
describe.only('Real API Submission', () => {
  ...
  it('should submit', async function() {
    ...
    expect(last['family/location'][0]['family/location/begin_repeat_s9iB96v5h'][0]['family/location/begin_repeat_s9iB96v5h/room']).toEqual('bedroom')
    expect(last['family/location'][0]['family/location/begin_repeat_s9iB96v5h'][1]['family/location/begin_repeat_s9iB96v5h/room']).toEqual('kitchen bathroom')
  ...
  it('should update non repeated questions', async function() {
    ...
    const updatedSubmissions = await sdk.v2.submission.get({formId: env.FORM_ID}).then(_ => _.results.find(_ => _._id === last._id))
